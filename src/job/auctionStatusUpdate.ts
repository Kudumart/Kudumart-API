            ...
            for (const auction of endedAuctions) {
                ...
                if (winnerId) {
                    ...
                    if (winner) {
                        ...
                        logger.info(`Auction ${auction.id} ended. Winner: ${winnerId}, Amount: ${winningBid}`);

                        // Send mail
                        let message = emailTemplates.auctionProductConfirmationNotification(winner, auction, winningBid);
                        try {
                            await sendMail(
                                winner.email,
