...
export const vendorRegister = async (
	...
	const { email, password, firstName, lastName, phoneNumber, platform } =
		req.body;
	...
	try {
		...
		try {
			...
			if (!platform || platform === "mobile") {
				...
				message = emailTemplates.verifyEmail(newUser, otp.otpCode);
			} else {
				//web
				...
				message = emailTemplates.verifyEmailWithLink(newUser, otpToken);
			}
...
export const verifyEmailWithToken = async (
	...
	try {
		...
		if (!user.email_verified_at) {
			...
			const hashedOtpToken = generateHashedOTPToken(token);
