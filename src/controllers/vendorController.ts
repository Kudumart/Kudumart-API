...
export const subscriptionPlans = async (
	...
	try {
		...
		logger.error(
			"Error fetching subscription plans or active subscription:",
			error,
		);
		res.status(500).json({
			message:
...
export const subscribeDollar = async (
	req: Request,
	res: Response,
): Promise<void> => {
	const vendorId = (req as AuthenticatedRequest).user?.id as string;
	...
	try {
		...
		const handleTransaction = async (amount: number) => {
			if (isWallet) {
				...
				await vendor.update(
					{ dollarWallet: vendor.dollarWallet - amount },
					{ transaction },
				);
			} else {
				const paymentGateway = await PaymentGateway.findOne({
 ...
 */
export const addBankInformation = async (
	req: Request,
	res: Response,
): Promise<void> => {
 ...
 */
export const getSingleBankInformation = async (
	req: Request,
	res: Response,
): Promise<void> => {
