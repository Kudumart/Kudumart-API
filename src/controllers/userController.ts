		...
		if (receiver?.fcmToken) {
			...
			const notificationMessage = {
				...
				data: {
					...
					type: PushNotificationTypes.NEW_MESSAGE,
				},
	...
	try {
					...
					if (customer.fcmToken) {
						const notificationMessage = {
							...
							data: {
								...
								type: PushNotificationTypes.ORDER_SHIPPED,
							},
		...
		if (buyer.fcmToken) {
			...
			try {
				...
				logger.error("Error sending push notification to buyer:", pushError);
			}
		}
		...
		if (vendor && vendor.fcmToken) {
			...
			const vendorNotificationMessage = {
				notification: {
					title: "Order Status Update",
					body: `The status of your product '${productData?.name}' has been updated to '${status}'.`,
				},
				data: {
					orderId: mainOrder.id.toString(),
					type: PushNotificationTypes.ORDER_STATUS_UPDATE,
			...
			try {
				await sendPushNotificationSingle(vendorNotificationMessage);
			} catch (pushError) {
				logger.error("Error sending push notification to vendor:", pushError);
			}
		}
