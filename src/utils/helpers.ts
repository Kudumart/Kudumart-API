...
const generateHashedOTPToken = (token: string): string => {
 ...
 */
const checkVendorProductLimit = async (
	vendorId: string,
	...
	try {
		...
		const subscriptionPlan = await SubscriptionPlan.findByPk(
		...
		if (productCount >= productLimit) {
			return {
				status: false,
				message:
					"You have reached the maximum number of products allowed for your current subscription plan. Please upgrade your plan to add more products.",
			};
		}
		...
		throw new Error(
			error.message || "An error occurred while checking the product limit.",
		);
	}
};

const checkVendorAuctionProductLimit = async (
	vendorId: string,
): Promise<{ status: boolean; message: string }> => {
	try {
		// Find the active subscription for the vendor
		...
		if (auctionProductLimit === null) {
			return {
				status: false,
				message:
					"Your subscription plan does not define a limit for auction products.",
			};
		}
		...
		if (auctionProductCount >= auctionProductLimit) {
			return {
				status: false,
				message:
					"You have reached the maximum number of auction products allowed for your current subscription plan. Please upgrade your plan to add more auction products.",
			};
		}
	...
	try {
		...
		if (maxAdsCount >= maxAds) {
			return {
				status: false,
				message:
					"You have reached the maximum number of adverts allowed for your current subscription plan. Please upgrade your plan to add more adverts.",
			};
		}
		...
		// Error type should be handled more gracefully if you have custom error types
		throw new Error(
			error.message || "An error occurred while checking the advert limit.",
		);
	}
};
	...
	const orderItem = await OrderItem.findOne({
		where: {
			...
			[Op.and]: Sequelize.literal(
				`JSON_UNQUOTE(JSON_EXTRACT(product, '$.id')) = '${productId}'`,
			),
		},
	});
